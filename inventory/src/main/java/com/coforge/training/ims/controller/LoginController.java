...
public class LoginController {
	...
	//Request --> (FC)Front Controller --> Controller --> Response(views-JSP)
	//Browse HomePage-  http://localhost:8085/ims/
	@RequestMapping("/")
	...
	public String saveDealer(HttpServletRequest req,@ModelAttribute("dealer") Dealer dealer) {
        ...
        lservice.saveDealer(dealer); //interacts with service layer for database logic implementation.
		return "index";
	}
	...
	public ModelAndView loginDealer(HttpServletRequest req,@ModelAttribute("dealer") Dealer dealer) {
        ...
        String s2 = st.nextToken(); //email id without domain
         ...
         Dealer d= lservice.findByEmail(email); //fetch record/object from two tables(Dealer & Address) based on emailid findbyemail
         if(d==null) {
